...
The one on the left contains all the configuration steps. 
...
The one on the right summarizes the input parameters (tuned along the 7 steps of
...
The names you give to columns in your CSV do not matter, because the WPM will create
...
This step is mandatory. It will be used in the plots' titles and in the output 
file names and also be concatenated with samples' IDs to limit sample confusions.
...
(where you specify the number of rows and columns by hand).
...
will see two blue boxes and a plate plan appear on the right summarizing all 
...
letters and numbers like in the example below):
...
> A __buffer well__ corresponds to wells filled filled with solution but do 
not biological material (*e.g.* to avoid/check for cross-contamination).
...
These are the spatial constraints that the WPM needs to create the plates.
...
side by side either row-wise or colmun-wise.
...
> A __fixed sample__ corresponds to quality control samples or standards,
...
When you start the WPM, a progress bar shows which iteration WPM is at.
...
# Using the WPM with the command lines {#R_commands}
...
* [buffer wells](#buffers) (wells where there will be solution without biological material)
* The position of [fixed samples](#fixed_wells)
* the [spatial constraint](#neighborhood) to place the samples
* the [maximal number of attemps](#iterations_number) for the WPM to find a valid solution
