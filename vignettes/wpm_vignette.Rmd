...
Make sure you are using the R 4.0.0 version or later.
For Windows users, we recommend using the Chrome browser rather than Internet 
Explorer.
...
First, you must upload the *__Comma-separated values__* CSV file. 
...
to plate maps (for more details on the ID see the [Results section](#results_panel) ).
...
> __`IMPORTANT`__ If after launching WPM and generating the results, you realize that one or more parameters do not work, you can always return to the "Parameters" tab and modify them. The data displayed in the "Results" tab will not be automatically changed, you will have to click again on the "start WPM" button to take into account the new changes.

> __`NOTE`__ If you want to create a new plate plan for another project, press `ctrl + f5`, this will reset the application.
