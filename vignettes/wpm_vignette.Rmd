...
abstract: >
    ...
    preventing plate edge effects) without programming skills.
vignette: >
...
The `wpm` package contains one unique function for the user that runs a shiny 
...
The section 1 is divided into 6 main steps, detailed below. This part is the most 
important to understand and master because it is here that we will specify all 
the constraints for generating the desired plate planes.
...
__IMPORTANT__: If WPM detects a problem or incompatibility between parameters, you 
will see an error message instead of the plate plan, explaining you what could 
be the problem.
...
Currently, there are 4 types proposed. Note that the modes are available only if 
they are compatible with the chosen blank mode.
...
The wells filled with not randomized wells will have the *"notRandom"* ID in the 
final dataset.
