...
server <- function(input, output, session) {
  ...
  distinct_gps <- shiny::reactive({
    ...
    shiny::validate(
      shiny::need(d_gp <= 12, message = "The number of separate groups must not exceed 12.")
    )
    return(d_gp)
  })
  ...
  #*****************************************************************************
    ...
    shiny::observeEvent(input$start_WPM_Btn,{
      ...
      shiny::validate(
        ...
        shiny::need(plate_specs$nb_lines > 0, "requires a number of rows greater than 0"),
        shiny::need(plate_specs$nb_cols > 0, "requires a number of columns greater than 0")
      )
